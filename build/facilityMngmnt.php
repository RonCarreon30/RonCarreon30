...
<body>
                            ...
                            <tbody id="facilityList" class="bg-white divide-y divide-gray-200">
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        echo "<tr id='facilityRow_" . $row["id"] . "'>";
                                        echo "<td class='px-3 py-4 whitespace-nowrap'><input type='text' id='building_" . $row["id"] . "' value='" . $row["building"] . "' class='bg-transparent border-none w-full' readonly></td>";
                                        echo "<td class='px-6 py-4 whitespace-nowrap'><input type='text' id='facilityName_" . $row["id"] . "' value='" . $row["facility_name"] . "' class='bg-transparent border-none w-full' readonly></td>";
                                        echo "<td class='px-6 py-4 whitespace-nowrap'><input type='text' id='descri_" . $row["id"] . "' value='" . $row["descri"] . "' class='bg-transparent border-none w-full' readonly></td>";
                                        echo "<td class='px-6 py-4 whitespace-nowrap text-right text-sm font-medium'>";
                                        echo "<button onclick='editFacility(" . $row["id"] . ")' class='inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2'>Edit</button>";
                                        echo "<button onclick='deleteFacility(" . $row["id"] . ")' class='ml-2 inline-flex justify-center rounded-md border border-red-500 shadow-sm px-4 py-2 bg-red-500 text-sm font-medium text-white hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2'>Delete</button>";
                                        echo "</td>";
                                        echo "</tr>";
                                    }
                                } else {
    ...
    function editFacility(userId) {
    var firstNameInput = document.getElementById('building_' + userId);
    var lastNameInput = document.getElementById('facilityName_' + userId);
    var emailInput = document.getElementById('descri_' + userId);
